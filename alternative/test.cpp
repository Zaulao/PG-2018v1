...
double diffuse(vec3 lightSource, Hit_record &rec) {
    ...
    // intersectLight.display();
    double dot = Vec3<double>::dotProduct(intersectLight, rec.normal);
...
bool hit (Ray *r, vector <Sphere> &objetosCena, double tmax, Hit_record &rec, double &perc, Sphere light, vec3 &colores) {
    ...
    for (int i = 0; i < objetosCena.size(); i++) {
        if (objetosCena.at(i).intersect(closest, record, r)) {
            ...
            rec.p = record.p;
            vec3 l = light.getPoint().operator-(r->getOrigin());
    ...
    if (notShadow) {
        //vec3 light(10, -15, -5);
        perc = diffuse(light.getPoint(), rec);
    }
    return hitAnything;
}
...
int main(){
    ...
    vec3 Ecenter (1, 1, 5); //red
    vec3 Ecenter2(0, 2, -5); //blue
    vec3 Ecenter3(4, 0, -5); //green
    vec3 Lcenter (6, 0, -2);
